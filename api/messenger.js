...
function getProductType(sessionId, context, cb){
	var url="https://www.cloudseed.com/generic?operation=getSchemaById&data="+JSON.stringify({Id:"Product"});
...
function searchProduct(sessionId, context, cb){
	var url="https://www.cloudseed.com/generic?operation=getSchemaById&data="+JSON.stringify({Id:"Product-"+context.productType});
	require('https').get(url, function(res) {
		var htmlString="";
		res.on('data', function(chunk){
			htmlString+=chunk;   
...
function getProducts(sessionId, context, cb){
	...
	//console.log(filters);
	var url="https://www.cloudseed.com/generic?operation=getSchemaRecords&data="+JSON.stringify(filters);
	require('https').get(url, function(res) {
		var htmlString="";
		res.on('data', function(chunk){
			htmlString+=chunk;   
		...
		res.on('end', function(){
			...
			function getProductImages(index){
				if(index==9){
					...
					var relurl="https://www.cloudseed.com/generic?operation=getRelatedRecords&data="+JSON.stringify({
						"org":"public",
...
function getProduct(sessionId, context, cb){
	var url="https://www.cloudseed.com/generic?operation=getSchemaRecordForView&data="+JSON.stringify({"org":"public","recordId":context.productId,"schema":"Product","dependentSchema":context.productType});
	require('https').get(url, function(res) {
		var htmlString="";
		res.on('data', function(chunk){
			htmlString+=chunk;   
		...
		while(i<context.products.length && i<9){
			...
			var element={
	            	...
	            	"buttons":[
	            	           {
	            	        	   "type":"web_url",
	            	        	   "url":"https://www.cloudseed.com/d/Product/"+context.products[i].value.name+"/"+context.products[i].id+"/?s=Product&ds="+context.subType,
	            	        	   "title":"View Item"
	            	           },
	            	           {
	           					"type":"postback",
			...
			for(var j=i;j<i+3;j++){
				...
				element.buttons.push({
					...
					//"url":"https://www.cloudseed.com/dtl/public/Product/"+context.products[j].id+"/?dependentSchema="+context.subType
					"type":"postback",
					"title":context.products[j].value.name,
						...
						"elements":[
						            {
						            	...
						            	"buttons":[
						            	           {
						            	        	   "type":"web_url",
						            	        	   "url":"https://www.cloudseed.com/d/Product/Product/"+context.productId+"/?s=Product&ds="+context.subType,
						            	        	   "title":"View Item"
						            	           },
						            	           {
						            	        	   "type":"web_url",
						            	        	   "url":"https://www.cloudseed.com/dtl/public/Product/"+context.productId+"/?s=Product&ds="+context.subType,
						            	        	   "title":"Check Price"
						            	           },
						            	           {
						            	        	   "type":"web_url",
		...
		message={
				"attachment":{
					...
					"payload":{
						...
						"elements":[
						            {
										title:"Welcome to cloudseed.com",
