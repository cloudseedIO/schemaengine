...
bucket.query(query, function(err, data) {
	...
	function updateProduct(index){
		...
		if(docu.docType=="Manufacturer"){
			...
			if(!docu.metaTitle){
				docu.metaTitle=docu.name+" | cloudseed.com";
			}
		...
		if(docu.docType=="ProductCategory"){
			...
			docu.metaTitle=docu.categoryName+" | cloudseed.com";
		...
		if(docu.docType=="MfrProCat"){
			...
			docu.metaTitle=docu.mfrName+" "+docu.categoryName+" | cloudseed.com";
		...
		if(docu.docType=="MfrProCatCity"){
			...
			docu.metaTitle=docu.manufacturerName+" "+docu.productCategoryName+" In "+docu.cityName+" | cloudseed.com";
		...
		if(docu.docType=="collection"){
			...
			if(docu.mfrName && docu.mfrName!=""){
				...
				docu.metaTitle = docu.mfrName+" - "+docu.collection+" | cloudseed.com";
				...
				docu.metaTitle = docu.collection+" | cloudseed.com";
		...
		if(docu.docType=="Provider"){
			...
			if(docu.address && docu.address.addressLocality && docu.address.addressLocality!=""){
				docu.metaTitle=docu.name+" | "+docu.address.addressLocality+" | cloudseed.com";
			}else{
				docu.metaTitle=docu.name+" | cloudseed.com";
			}
		...
		if(docu.docType=="Product"){
			...
			//}
			docu.metaTitle=docu.esMeta.trim()+" "+ docu.name.trim().substr(0.40)+" "+docu.mfrProductNo.trim()+" | cloudseed.com";
		...
		if(docu.docType=="ServiceProvider"){
			...
			Title+=docu.name+" | cloudseed.com";
			docu.metaTitle=Title;
			docu.metaDescription=docu.name+": "+(specialities?("Experts in "+specialities+". "):"")+"Browse through our portfolio of projects. We offer best prices for cloudseed customers.";
		}
