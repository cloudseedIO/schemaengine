...
//var nlp = require('./NlpProcesser.js');
...
/*
 * dataParseTemplate={ "Schema":"Product", "DependentSchema":"Product-Fabric",
 * "prefillData":{ "productCategory":"productCategoryId",
 * "Manufacturer":"Manufacturer68de3c3e-efee-466b-54cb-1c4b2b61d5ba",
 * "esMeta":"manufacturerName" }, "properties":{ //"Manufacturer":{ // "id":
 * "Manufacturer68de3c3e-efee-466b-54cb-1c4b2b61d5ba", // "name": "C&C Milano"
 * //}, "name":{ "value":"name", "operations":["trim"] }, "groupID":{
 * "value":"name", "operations":["matchWithRegex"], "regex":/(adsf)/ },
 * "productImages":{ "type":"image", "value":"productImages",
 * "operations":["generateImageStructure"] } }, "dependentProperties":{
 * //"Manufacturer":{ // "id":
 * "Manufacturer68de3c3e-efee-466b-54cb-1c4b2b61d5ba", // "name": "C&C Milano"
 * //}, /*"pattern":{ "value":"pattern", "operations":["matchWithRegex"],
 * "regex":/(?:^|)(Animal
 * Print)(?=\W)|(?:^|)(Abstract)(?=\W)|(?:^|)(Botanical)(?=\W)|(?:^|)(Damask)(?=\W)|(?:^|)(Deco)(?=\W)|(?:^|)(Graphic)(?=\W)|(?:^|)(Herringbone)(?=\W)|(?:^|)(Organic)(?=\W)|(?:^|)(Paisley)(?=\W)|(?:^|)(Plaid)(?=\W)|(?:^|)(Plain)(?=\W)|(?:^|)(Scenic)(?=\W)|(?:^|)(Solid)(?=\W)|(?:^|)(Stripes)(?=\W)|(?:^|)(Texture)(?=\W)|(?:^|)(Other)(?=\W)/gi },
 * "material":{ "value":"material", "operations":["matchWithRegex"],
 * "regex":/(?:^|)(Cupro)(?=\W)|(?:^|)(Non
 * woven)(?=\W)|(?:^|)(Viscose)(?=\W)|(?:^|)(Other)(?=\W)|(?:^|)(Spun)(?=\W)|(?:^|)(Polyurethane)(?=\W)|(?:^|)(Composite)(?=\W)|(?:^|)(Silk)(?=\W)|(?:^|)(Wool)(?=\W)|(?:^|)(Rayon)(?=\W)|(?:^|)(Acrylic)(?=\W)|(?:^|)(Cotton)(?=\W)|(?:^|)(Polyester)(?=\W)|(?:^|)(Linen)(?=\W)|(?:^|)(Leather)(?=\W)/gi //
 * /(?:^|)(Cupro)(?=\s)|(?:^|)(Non
 * woven)(?=\s)|(?:^|)(Viscose)(?=\s)|(?:^|)(Other)(?=\s)|(?:^|)(Spun)(?=\s)|(?:^|)(Polyurethane)(?=\s)|(?:^|)(Composite)(?=\s)|(?:^|)(Silk)(?=\s)|(?:^|)(Wool)(?=\s)|(?:^|)(Rayon)(?=\s)|(?:^|)(Acrylic)(?=\s)|(?:^|)(Cotton)(?=\s)|(?:^|)(Polyester)(?=\s)|(?:^|)(Linen)(?=\s)|(?:^|)(Leather)(?=\s)/gi //
 * /(Leather)?(Linen)?(Polyester)?(Cotton)?(Acrylic)?(Rayon)?(Wool)?(Silk)?(Spun)?(Polyurethane)?(Composite)?(Cupro)?(Non
 * woven)?(Viscose)?(Other)?[a-zA-z]+/g //
 * /(?:^|)(Cotton)(?=)|(?:^|)(Polyster)(?=)/g } } };
 */
/*
 * var record = {"name":"F3168025 LOFT",
 * "sourceUrl":"http://www.pierrefrey.com/uk/produit/tissus/247-F3168025-loft.htm",
 * "pattern":"\n \tCottons, Plain\n\t", "material":"\n\tCotton : 100% \n ",
 * "description":"\n \tThis vintage looking cotton twill is an essential in this
 * collection. 100% cotton and washable, it is available in over twenty colors
 * ranging from natural, pastels and tangy tones and can be used for both
 * curtains and upholstery. ", "abrasion test":"Abrasion test 60.000 T",
 * "specifications-text":"Composition,\n\tCotton : 100% \n
 * ,Colour:,\n\t\tCoquelicot \n ,Style:,\n\t\tContemporary \n ,Technical
 * information,\n\t\tWeight : 450 gr \n\t\tWidth: 140 cm / 55.11 inch \n\t\t\t\t
 * \n ,Abrasion test 60.000 T", "specifications-use":"Hand wash only,Chlorine
 * prohibited,Ironing 110? max.,All dry cleaning solvents,Tumble drying
 * prohibited,Seat fabric designed for Normal use","care instructions":"Hand
 * wash only,Chlorine prohibited,Ironing 110? max.,All dry cleaning
 * solvents,Tumble drying prohibited", "specificities - use":"Seat fabric
 * designed for Normal use",
 * "image":"http://media.pierrefrey.com/tissus/403_F3168025.jpg" };
 * 
 */

function processRecord(postProcessorJson, scrappedRecs, callback) {
...
function processAndStoreRecord(scrappedRecord, callback) {
	...
	if(!dataParseTemplate.test){
	uploadToCloudinary(newRecord4, function(newRecord5) {
	 saveRecord(newRecord5, function(rec, res, reason) {
	 if (res) {
	 console.log("Record Saved.......",rec);
	 } else {
	 console.log(reason);
	 }
	 callback(rec, res, reason);
	...
	/*
	 * newRecord ={ "name": record.name, "sourceUrl":record.sourceUrl,
	 * "mfrProductNo":record.mfrProductNo, "description":record.description,
	 * "productImages":(record.productImages &&
	 * Array.isArray(record.productImages) && record.productImages.length>0 )
	 * ?generateImages(record.productImages,"image",record):(record.productImages &&
	 * record.productImages.length>0?generateImages([record.productImages],"image",record):[]),
	 * "attachments":(record.attachments && Array.isArray(record.attachments) &&
	 * record.attachments.length>0 )
	 * ?generateImages(record.attachments):(record.attachments &&
	 * record.attachments.length>0?generateImages([record.attachments]):[]),
	 * "recordId":recordId , "org": "public", "docType": "Product", "author":
	 * user, "editor": user, "dateCreated": global.getDate(), "dateModified":
	 * global.getDate(), "revision": 1, "@superType": "Product", "@identifier":
	 * "name", "@derivedObjName":
	 * (dependentSchema?(dependentSchema):""),//"Product-Fabric",//have to add
	 * manually "productType":
	 * (dependentSchema?(dependentSchema.replace("Product-","")):""),//have to
	 * add manually "relationDesc": [ "Manufacturer-manufactures-recordId",
	 * "recordId-manufacturedBy-Manufacturer", "collection-hasProduct-recordId",
	 * "recordId-ofCollection-collection",
	 * "productCategory-hasProduct-recordId", "recordId-ofType-productCategory",
	 * "groupID-has-recordId" ], "productCategory":record.productCategory,
	 * "cloudPointHostId": "schemaengine", "collection":record.collection,
	 * "specifications":record.specifications?record.specifications:"",
	 * "dependentProperties":setDependentProperties(record), "$status":
	 * "underReview", "@uniqueUserName":
	 * record.name.replace(/\W/g,"-").toLowerCase(),
	 * "esMeta":Manufacturer.name,//update fields manually
	 * "Manufacturer":Manufacturer.id,//"Manufacturer68de3c3e-efee-466b-54cb-1c4b2b61d5ba",//update
	 * fields manually "metaTitle":generateMeta(record)+" |schemaengine.com",
	 * "metaDescription":generateMeta(record)+". Find and chat with local
	 * suppliers and dealers near you.", "record_header":Manufacturer.name+"
	 * "+record.name };
	 * 
	 */
}
...
function setSystemProps(newRecord, schema, dependentSchema, callback) {
	...
	newRecord['cloudPointHostId'] = "schemaengine";
...
function setEsTitleNMeta(record, callback){
	...
	//}
	record.metaTitle=record.esMeta.trim()+" "+ record.name.trim().substr(0.40)+" "+record.mfrProductNo.trim()+" | schemaengine.com";
	...
	record["@uniqueUserName"]=userName;
	record.metaTitle=record.esMeta.trim()+" "+ record.collection.trim().substr(0.40)+" | schemaengine.com";
...
var globalOperations = {
	...
	// console.log(finalValue);
		if(replaceText[i].contains("/g")){
			var regExStr = replaceText[i].replace(/\//g, '');
			//regExStr = regExStr.splice();
			replaceText[i]= new RegExp(replaceText[i]);
		}
	...
	addTextAtBeginning : function(scrappedRecord, propertyJson, callback) {
	var value = scrappedRecord[propertyJson.value];
	var text = propertyJson.text;
	if(value.constructor == String){
	callback(text+value);
