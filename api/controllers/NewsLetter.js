...
function getMailData(data,callback){
	...
	getMfrMessageComponents(data.manufacturer.recordId,components,function(res){
		...
		//to=["sidhartha@cloudseed.io"]//,"sidhartha@cloudseed.io","editorial@cloudseed.com"];
							...
							"<div style='margin: 0 auto;text-align: center;'>" +
								...
								(data.includeUnsubscribe?("<div class='row' style='text-align: left;margin-bottom: 10px;'>" +
									"<span style='font-size: 13px;color: black;'>" +
										"<a href='https://www.cloudseed.com/newsletter?operation=unsubscribe&recordId="+data.manufacturer.recordId+"'>unsubscribe here</a>" +
									"</span>" +
								"</div>"):"")+
							"</div>" +
						"</body>" +
...
function getMfrComponent(mfrRecord,component,callback){
	if(component=="MFR_SLUG"){
		if(mfrRecord["@uniqueUserName"]){
			callback({html:"<a href='https://www.cloudseed.com/"+mfrRecord["@uniqueUserName"].toLowerCase()+"'>cloudseed.com/"+mfrRecord["@uniqueUserName"].toLowerCase()+"</a>"});
		}else{
			callback({html:""});
		}
					...
					try{
						...
						html+="<div style='padding: 15px;float: none;display: inline-block;'>" +
									(catDocs[recCount.productCategory]["@uniqueUserName"]?(
										"<a style='color:black;text-decoration:none;' href='https://www.cloudseed.com/"+catDocs[recCount.productCategory]["@uniqueUserName"]+"'>"
									):"")+
									"<div>"+
										img +
									"</div>"+
				...
				try{
					...
					html+="<div style='padding: 15px;float: none;display: inline-block;'>" +
								(product["@uniqueUserName"]?(
										"<a style='color:black;text-decoration:none;' href='https://www.cloudseed.com/"+product["@uniqueUserName"]+"'>"
								):"")+
								"<div>"+
									img +
								"</div>"+
