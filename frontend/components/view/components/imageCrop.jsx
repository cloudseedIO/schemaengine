...
var isTouchDevice = ""; //  navigator.msMaxTouchPoints > 0;//; 'ontouchstart' in window ||
var draggableEvents = {
  mobile: {
    react: {
      down: "onTouchStart",
...
var deviceEvents = isTouchDevice
  ? draggableEvents.mobile
...
var ReactAvatarEditor = React.createClass({
  ...
  componentDidMount: function() {
    ...
    document &&
      document.addEventListener(
        deviceEvents.native.move,
        this.handleMouseMove,
        false
      );
    document &&
      document.addEventListener(
        deviceEvents.native.up,
        this.handleMouseUp,
        false
      );
  ...
  paintImage: function(context, image) {
    if (image.resource) {
      ...
      if (position.width < 200) {
        var x = image.x + (200 - position.width) / 2;
        context.drawImage(
          image.resource,
          x,
          image.y,
      ...
      } else if (position.height < 200) {
        var y = image.y + (200 - position.height) / 2;
        context.drawImage(
          image.resource,
          image.x,
          y,
  ...
  calculatePosition: function(image) {
    ...
    y =
      fromBottom > dimensions.height ? y : y + (dimensions.height - fromBottom);
  ...
  handleMouseMove: function(e) {
    ...
    var mousePositionX = isTouchDevice
      ? event.targetTouches[0].pageX
      : e.clientX;
    var mousePositionY = isTouchDevice
  ...
  getBoundedX: function(x, scale) {
    ...
    var rightPoint = Math.ceil(
      -image.width * scale + dimensions.width + dimensions.border
    );
    var leftPoint = dimensions.border;

    var result;
    if (x - widthDiff >= dimensions.border)
      result = dimensions.border + widthDiff;
  ...
  getBoundedY: function(y, scale) {
    ...
    var bottomPoint = Math.ceil(
      -image.height * scale + dimensions.height + dimensions.border
    );
    var topPoint = dimensions.border;

    var result;
    if (y - heightDiff >= dimensions.border)
      result = dimensions.border + heightDiff;
...
exports.ReactAvatarEditor = ReactAvatarEditor;
