...
function getHandleBarMetaData(request,config){
	...
	return {
			...
			title:config.title?config.title:"Cloudseed",
			logo:config.logo?config.logo:"cloudseed.png",
			favIcon:config.favIcon?config.favIcon:config.logo?config.logo:"cloudseed.png",
			loader:config.loader?config.loader:"cloudseedLoader.gif",
			...
			lpi:config.landingTemplate?config.landingTemplate:"CloudseedTemplate",
			cloudPointHostId:config.cloudPointHostId?config.cloudPointHostId:"cloudseed",
...
function getFavicon(hostname,callback){
	...
	var logo=(hostconfig && hostconfig.logo)?hostconfig.logo:"cloudseed.png";
...
function getLandingPageMeta(hostname,lpi,handleBarMeta,config){
	...
	var meta="";
	var image_src="";
	if(config && config.cloudPointHostId && config.cloudPointHostId=="wishkarma"){
	...
	try{
		meta  = "<title>"+mtitle+"</title>\n" +
				"<meta name='author' content='"+ (config.title?config.title:"Cloudseed") +"'/>\n" +
...
function serveChangePasswordPage(request,response){
	...
	getCloudPointConfig(hostname,function(config){
		...
		function sendResponse(){
			...
			var meta="";
			var mtitle=(handleBarMeta.title?handleBarMeta.title:"Cloudseed")+": Reset Password";
			try{
				meta = "<title>"+mtitle.replace(metaCleanRegEx,' ')+"</title>\n" +
						"<meta name='author' content='"+ (handleBarMeta.title?handleBarMeta.title:"Cloudseed") +"'/>\n";
...
function serveActivatePage(request,response){
	...
	getCloudPointConfig(hostname,function(config){
		...
		function sendResponse(){
			...
			stateString=JSON.stringify(stateString);
			var meta="";
			var mtitle=(handleBarMeta.title?handleBarMeta.title:"Cloudseed")+": Setup Your Account";
			try{
				meta = "<title>"+mtitle.replace(metaCleanRegEx,' ')+"</title>\n" +
						"<meta name='author' content='"+ (handleBarMeta.title?handleBarMeta.title:"Cloudseed") +"'/>\n";
			}catch(err){}
...
function serveJoinPage(request,response){
	...
	getCloudPointConfig(hostname,function(config){
		...
		function sendResponse(){
			...
			stateString=JSON.stringify(stateString);
			var meta="";
			var mtitle=(handleBarMeta.title?handleBarMeta.title:"Cloudseed")+": Join Wishkarma";
			try{
				meta = "<title>"+mtitle.replace(metaCleanRegEx,' ')+"</title>\n" +
						"<meta name='author' content='"+ (handleBarMeta.title?handleBarMeta.title:"Cloudseed") +"'/>\n";
...
function servePageNotFound(request,response){
	...
	getCloudPointConfig(hostname,function(config){
		...
		function sendResponse(){
			...
			var meta="";
			var mtitle=(handleBarMeta.title?handleBarMeta.title:"Cloudseed")+": Page not found";
			try{
				meta = "<title>"+mtitle.replace(metaCleanRegEx,' ')+"</title>\n" +
						"<meta name='author' content='"+ (handleBarMeta.title?handleBarMeta.title:"Cloudseed") +"'/>\n";
...
function serveExplorePage(request,response){
	...
	getCloudPointConfig(hostname,function(config){
		WebAPI.getExploreUniqueUserName(request.body,function(searchTextDoc){
			...
			var meta="";
			var mtitle=(handleBarMeta.title?handleBarMeta.title:"Cloudseed")+": Explore | "+searchTextId;
...
function serveAdminSpecificPage(request,response){
	if(typeof request!="undefined" && ((typeof request.session!="undefined" && typeof request.session.userData!="undefined") || request.originalUrl.indexOf("manufacturerInfo")!=-1)){
		...
		getCloudPointConfig(hostname,function(config){
			...
			var meta="";
			var mtitle=(handleBarMeta.title?handleBarMeta.title:"Cloudseed")+": Admin related page";
			try{
				meta = "<title>"+mtitle.replace(metaCleanRegEx,' ')+"</title>\n" +
				"<meta name='author' content='"+ (handleBarMeta.title?handleBarMeta.title:"Cloudseed") +"'/>\n";
			}catch(err){}
...
function serveOrgPage(request,response){
	...
	getCloudPointConfig(hostname,function(config){
		...
		var meta="";
		var mtitle=(handleBarMeta.title?handleBarMeta.title:"Cloudseed")+": Org page";
		try{
			meta = "<title>"+mtitle.replace(metaCleanRegEx,' ')+"</title>\n" +
			"<meta name='author' content='"+ (handleBarMeta.title?handleBarMeta.title:"Cloudseed") +"'/>\n";
		}catch(err){}
