...
cluster.query(query,["MfrProCat" ,"%faucet%"],function(err,result){
	...
	function process(index){
		...
		cluster.query(query2,[doc.ProductCategory] ,function(err, result2) {
			if(err){
				console.log(err);
			}else{
			...
			cluster.query(query3,[doc.Manufacturer] ,function(err, result3) {
				if(err){
					console.log(err);
				}else{
				...
				cluster.query(query4,[doc.recordId,doc.categoryName,doc.mfrProCatName,doc["@uniqueUserName"]] ,function(err, result4) {
					console.log(result4);
					process(index+1);
				});
