...
function processAndStoreRecord(scrappedRecord, callback) {
	 ...
	 * "cloudPointHostId": "wishkarma", "collection":record.collection,
	 ...
	 * fields manually "metaTitle":generateMeta(record)+" |Wishkarma.com",
...
function setSystemProps(newRecord, schema, dependentSchema, callback) {
	...
	newRecord['cloudPointHostId'] = "wishkarma";
...
function setEsTitleNMeta(record, callback){
	...
	//}
	record.metaTitle=record.esMeta.trim()+" "+ record.name.trim().substr(0.40)+" "+record.mfrProductNo.trim()+" | Wishkarma.com";
	...
	record["@uniqueUserName"]=userName;
	record.metaTitle=record.esMeta.trim()+" "+ record.collection.trim().substr(0.40)+" | Wishkarma.com";
