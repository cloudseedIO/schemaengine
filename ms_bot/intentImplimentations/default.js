    ...
    if(!session.message.text && session.message.value && session.message.value.type){
    	if(session.message.value.type=="DiscoverProducts"){
		 ...
		 /*builder.Prompts.choice(
		  	session, 
		  	"Welcome to schemaengine! What are you looking for?", 
		  	["Products","Suppliers"],
		 
		  );*/ 
		  // 	{ listStyle: builder.ListStyle.button }
		  session.send("Welcome to schemaengine! What are you looking for? eg: Show Grohe faucets, Find Duravit suppliers, I am looking for duravit sinks");
		  session.endDialog();
    }
}]
